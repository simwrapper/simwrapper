...
onmessage = async ({ data: { config, filePath, fileSystem } }: MessageEvent) => {
  ...
  try {
    ...
    // console.log(100, { _zoneData, _dailyZoneData, _dailyLinkData })
...
async function processFile(filePath: string, fileSystem: FileSystemConfig) {
  try {
    console.log(filePath)

    const stream = await _fileAPI.getFileStream(filePath)
