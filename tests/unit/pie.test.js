...
import Pie from '../../src/dash-panels/pie.vue'
...
Object.defineProperty(window, 'Blob', Blob)
...
global.fetch = vi.spyOn(window.URL, 'createObjectURL').mockImplementation(() => 'http://fake.url')
...
describe('Pie.vue', () => {
  ...
  test('correct test: pie chart', async () => {
    vi.spyOn(window.URL, 'createObjectURL').mockImplementation(() => 'http://fake.url')
    // Plotly.react.mockResolvedValue(null)
    ...
    // const rootByCss = wrapper.findComponent('.myplot')
    // expect(rootByCss.vm.$options.name).toBe('PieChartPanel')
  })
})
