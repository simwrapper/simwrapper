...
.click-through-times-component
  .label-area
    ...
    .buttons-row
      ...
      button.button.play-button2(size="is-small" type="is-link"
      @click="getNextHour") {{ '>' }}
    ...
    methods: {
        ...
        getPreviousHour() {
            if (this.state.allTimesIndex > 0) {
                ...
                this.state.label = this.convertSecondsToClockTimeMinutes(this.$props.allTimes[this.state.allTimesIndex])
            } else {
                this.state.label = this.convertSecondsToClockTimeMinutes(this.$props.allTimes[0])
        ...
        updateData() {
            ...
            this.state.timeLabel = [
                this.convertSecondsToClockTimeMinutes(this.allTimes[0]),
            ]
