...
import { getEnhancedError, hasBigInts, PLUGINS_BY_FILTER_ID, sanitizeBigInts } from './utils'
...
export class H5WasmLocalFileApi extends DataProviderApi {
  ...
  public constructor(
    ...
    private readonly getPlugin?: (name: Plugin) => Promise<ArrayBuffer | undefined>
  ) {
    super(file.name)
  ...
  public override getExportURL<D extends Dataset<ArrayShape>>(
    ...
    const url = this._getExportURL?.(format, dataset, selection)
    if (url) {
      return url
    }
  ...
  private async processFilters(filters: Filter[] = []): Promise<void> {
    const pluginsToLoad = filters.map(({ id }) => PLUGINS_BY_FILTER_ID[id]).filter(isDefined)
