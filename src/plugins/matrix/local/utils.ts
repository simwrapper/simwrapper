import { assertDefined, assertNonNull, isNumericType } from './guards'
import type { Attribute, ChildEntity, DType, Group, ProvidedEntity, Shape } from './hdf5-models'
import { DTypeClass, Endianness, EntityKind, H5TClass } from './hdf5-models'
import {
  arrayType,
  bitfieldType,
  buildEntityPath,
...
export function parseEntity(
  ...
  return {
    ...baseEntity,
    attributes: [],
    kind: h5wEntity instanceof H5WasmDatatype ? EntityKind.Datatype : EntityKind.Unresolved,
  }
}
...
export function parseDType(metadata: Metadata): DType {
  ...
  if (h5tClass === H5TClass.Compound) {
    ...
    return compoundOrCplxType(
      Object.fromEntries(compound_type.members.map(member => [member.name, parseDType(member)]))
    )
  }
...
export function readSelectedValue(h5wDataset: H5WasmDataset, selection: string | undefined) {
  if (!selection) {
...
export function hasBigInts(type: DType): boolean {
  ...
  return (
    (type.class === DTypeClass.Integer || type.class === DTypeClass.Unsigned) && type.size === 64
  )
}
...
const DIAG_PREDICATES: ((diag: HDF5Diag) => boolean)[] = [
  (diag: HDF5Diag) => {
    return diag.major === 'Data filters' && /^required filter.*not registered$/iu.test(diag.message)
  },
]
