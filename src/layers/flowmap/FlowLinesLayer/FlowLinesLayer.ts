...
import FragmentShader from './FlowLinesLayerFragment.glsl.frag?raw'
import VertexShader from './FlowLinesLayerVertex.glsl.vert?raw'
...
class FlowLinesLayer<F> extends Layer {
  ...
  updateState({ props, oldProps, changeFlags }: Record<string, any>) {
  ...
  draw(): void {
    const { outlineColor, thicknessUnit, gap, opacity } = this.props as any
    // gl.lineWidth(1)
  ...
  _getModel(): Model {
    ...
    const { drawOutline, outlineThickness } = this.props as any
