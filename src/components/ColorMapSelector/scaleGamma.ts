...
import type { InterpolatorFactory, NumberValue, ScalePower } from 'd3-scale'
import { isNumber } from 'lodash'

export type Domain = [number, number]

import type { ScaleGammaConfig } from './vismodels'
...
export function scaleGamma(config?: ScaleGammaConfig): ScaleGamma {
  ...
  const scaleFn = function (val: NumberValue) {
    ...
    return _interpolate(..._range)(normalize(..._domain)(clamper(x)) ** _exponent)
  }
  ...
  function invert(val: Output) {
    return clamper(interpolateNumber(..._domain)(normalize(..._range)(val) ** (1 / _exponent)))
  }
  ...
  function tickFormat(count?: number | undefined, specifier?: string | undefined) {
