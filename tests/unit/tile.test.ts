import { shallowMount } from '@vue/test-utils'
import { vi } from 'vitest'
import '../../src/shims-vue.d'
import * as d3color from 'd3-color'

import blobUtil from 'blob-util'

// import Tile from '../../src/dash-panels/tile.vue'
import Tile from '@/dash-panels/tile.vue'
import DashboardDataManager from '../../src/js/DashboardDataManager'

// app icon
const blob = blobUtil.base64StringToBlob(
  ''
)

// Defines the returned csv text for the first mock
const csvText1 = `custom Image; 1234; vsp_logo.png; https://vsp.berlin
custom Image (parent folder); text; ../warning.png
image from assets Folder (Material UI); 3517; emoji_transportation
Fontawesome Icon 1; 75135; virus-covid
Fontawesome Icon 2; transport; departure_board`

// Defines the returned csv text for the second mock
const csvText2 = `custom Image; 1234; vsp_logo.png; https://vsp.berlin
custom Image (parent folder); text; ../warning.png
image from assets Folder (Material UI); 3517; emoji_transportation`

// Defines the fetch mock for the first test
const fetchMock1 = vi.fn().mockResolvedValue({
  status: 200,
  text: () => Promise.resolve(csvText1),
  blob: () => Promise.resolve(blob),
})

// Defines the fetch mock for the second test
const fetchMock2 = vi.fn().mockResolvedValue({
  status: 200,
  text: () => Promise.resolve(csvText2),
  blob: () => Promise.resolve(blob),
})

// Defines the number of tiles for the tests
let numberOfTilesTest1 = 5
let numberOfTilesTest2 = 3

describe('Tile.vue', () => {
  beforeEach(() => {
    global.fetch = fetchMock1
  })

  test('correct test: 5 tiles', async () => {
    const wrapper = shallowMount(Tile, {
      propsData: {
        fileSystemConfig: {
          baseURL: 'http://localhost:8000',
          description: 'Files on this computer, shared using "simwrapper serve" tool',
          name: 'local',
          slug: 'local',
          thumbnail: '/simwrapper/images/thumb-localfiles.jpg',
        },
        subfolder: '/tiles-test',
        files: ['dashboard.yaml', 'simwrapper-config.yaml', 'warning.png'],
        cardId: 'cardId',
        cardTitle: 'cardTitle',
        config: {
          dataset: 'data/tiles_new_format.csv',
          height: 0.1,
          id: 'card-id-1',
          isLoaded: false,
          number: 1,
          title: 'Tiles Plug-in Example New Format',
          type: 'tile',
        },
        datamanager: new DashboardDataManager('local', '/'),
      },
    })

    // Workaround for the async/await stuff...
    for (let i = 0; i < 10; i++) {
      await wrapper.vm.$nextTick()
    }

    // Check if the component exists
    expect(wrapper.exists()).toBe(true)

    // Check if the component is not empty
    const rootByCss = wrapper.findComponent(Tile)
    expect(rootByCss.vm.$options.name).toBe('Tile')

    // Create all wrapper elemnts
    const contentWrapper = wrapper.find('.content')
    const tilesContainerWrapper = contentWrapper.find('.tiles-container')
    const tile = tilesContainerWrapper.findAll('.tile')

    // Check the number of tiles
    expect(tile.length).toBe(numberOfTilesTest1)

    // Check the css-classes
    for (let i = 0; i < numberOfTilesTest1; i++) {
      expect(tile.at(i).attributes().class).toBe('tile')
    }

    // Check the background colors
    for (let i = 0; i < numberOfTilesTest1; i++) {
      expect(tile.at(i).attributes().style).toBe(
        'background-color: ' + d3color.rgb(wrapper.vm.colors[i]) + ';'
      )
    }

    // Check the title and the value
    for (let i = 0; i < numberOfTilesTest1; i++) {
      expect(tile.at(i).text()).toBe(
        wrapper.vm.dataSet.data[i][wrapper.vm.tileNameIndex] +
          wrapper.vm.dataSet.data[i][wrapper.vm.tileValueIndex]
      )
    }
  })

  test('correct test: 3 tiles', async () => {
    global.fetch = fetchMock2

    const wrapper = shallowMount(Tile, {
      propsData: {
        fileSystemConfig: {
          baseURL: 'http://localhost:8000',
          description: 'Files on this computer, shared using "simwrapper serve" tool',
          name: 'local',
          slug: 'local',
          thumbnail: '/simwrapper/images/thumb-localfiles.jpg',
        },
        subfolder: '/tiles-test',
        files: ['dashboard.yaml', 'simwrapper-config.yaml', 'warning.png'],
        cardId: 'cardId',
        cardTitle: 'cardTitle',
        config: {
          dataset: 'data/tiles_new_format.csv',
          height: 0.1,
          id: 'card-id-1',
          isLoaded: false,
          number: 1,
          title: 'Tiles Plug-in Example New Format',
          type: 'tile',
        },
        datamanager: new DashboardDataManager('local', '/'),
      },
    })

    // Workaround for the async/await stuff...
    for (let i = 0; i < 10; i++) {
      await wrapper.vm.$nextTick()
    }

    // Check if the component exists
    expect(wrapper.exists()).toBe(true)

    // Check if the component is not empty
    const rootByCss = wrapper.findComponent(Tile)
    expect(rootByCss.vm.$options.name).toBe('Tile')

    // Create all wrapper elemnts
    const contentWrapper = wrapper.find('.content')
    const tilesContainerWrapper = contentWrapper.find('.tiles-container')
    const tile = tilesContainerWrapper.findAll('.tile')

    // Check the number of tiles
    expect(tile.length).toBe(numberOfTilesTest2)

    // Check the css-classes
    for (let i = 0; i < numberOfTilesTest2; i++) {
      expect(tile.at(i).attributes().class).toBe('tile')
    }

    // Check the background colors
    for (let i = 0; i < numberOfTilesTest2; i++) {
      expect(tile.at(i).attributes().style).toBe(
        'background-color: ' + d3color.rgb(wrapper.vm.colors[i]) + ';'
      )
    }

    // Check the title and the value
    for (let i = 0; i < numberOfTilesTest2; i++) {
      expect(tile.at(i).text()).toBe(
        wrapper.vm.dataSet.data[i][wrapper.vm.tileNameIndex] +
          wrapper.vm.dataSet.data[i][wrapper.vm.tileValueIndex]
      )
    }
  })
})
