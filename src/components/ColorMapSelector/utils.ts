...
import type { Domain, NumArray } from './models-vis'
import { getDims, ScaleType, toTypedNdArray } from './models-vis'
...
import type { CustomDomain, DomainErrors } from './vismodels'
import { DomainError } from './vismodels'
import { getAxisValues, SCALES_VALID_MINS } from './models-vis'
import { INTERPOLATORS } from './interpolators'
import type { ColorMap, D3Interpolator, TextureSafeTypedArray } from './models'

const GRADIENT_PRECISION = 1 / 20
export const GRADIENT_RANGE = range(0, 1 + GRADIENT_PRECISION, GRADIENT_PRECISION)
...
export function getVisDomain(customDomain: CustomDomain, dataDomain: Domain): Domain {
...
function getColorStops(interpolator: D3Interpolator, minMaxOnly: boolean): string {
  if (minMaxOnly) {
...
export function getPixelEdgeValues(rawValues: NumArray | undefined, pixelCount: number): NumArray {
...
export function scaleDomain(domain: Domain, scaleType = ScaleType.Linear): Domain {
  const scaleFn = SCALE_FNS[scaleType]
  return [scaleFn(domain[0]), scaleFn(domain[1])]
}

export function toTextureSafeNdArray(ndArr: NdArray<NumArray>): NdArray<TextureSafeTypedArray>
...
export function getDataTexture(values: undefined, magFilter?: TextureFilter): undefined
export function getDataTexture(
