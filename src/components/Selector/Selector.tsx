import styles from '@/components/Selector/Selector.module.css'
...
function Selector<T extends string>(props: Props<T>) {
  const { label, value, disabled, onChange, options, optionComponent: Option } = props
      ...
      <Wrapper className={styles.wrapper} onSelection={onChange}>
        ...
        >
          {Array.isArray(options) ? (
            <OptionList optionList={options} optionComponent={Option} value={value} />
          ) : (
            <ul className={styles.list}>
              {Object.entries(options).map(([groupLabel, groupOptions]) => (
                <li key={groupLabel}>
                  ...
                  <ul className={styles.list}>
                    <OptionList optionList={groupOptions} optionComponent={Option} value={value} />
                  </ul>
                </li>
              ))}
            </ul>
