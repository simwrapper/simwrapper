...
interface VizDetail {
  ...
  radius: number
  colorRamp: Ramp
  // breakpoints: number[] | { values: number[]; colors: string[] } | null
}
...
export interface Ramp {
  ...
  steps: number
  breakpoints?: number[]
  customRamp?: string[] // use this one if its set. if not use ramp
}

// TODO: they have to work: bathymetry, electric, inferno, jet, magma, par, viridis
...
const DEFAULT_NUMBER_OF_BUCKETS = 7 // Info: buckets and breakpoints are used interchangeably
...
const MyComponent = defineComponent({
  ...
  data() {
    return {
      guiConfig: {
        ...
        'clip max': 100,
        'color ramp': 'viridis', // TODO
        // colorRamps: ['bathymetry', 'electric', 'inferno', 'jet', 'magma', 'par', 'viridis'], // TODO
        colorRamps: ALL_COLOR_RAMPS.map(ramp => ramp.ramp),
        flip: false,
        // @ts-ignore ->
      ...
      vizDetails: {
        ...
        radius: DEFAULT_RADIUS,
        // colorRamp: 'viridis',
        colorRamp: { ramp: 'Viridis', style: Style.sequential, reverse: true } as Ramp,
        flip: false,
        // breakpoints: null as number[] | { values: number[]; colors: string[] } | null,
      } as VizDetail,
  ...
  async mounted() {
    // console.log("Hey there! I'm the XYTimeV2 plugin.")
    ...
    // Load the viz details from the YAML config file
  ...
  methods: {
    ...
    finishedLoadingData(totalRows: number, data: any) {
      //   console.log('finishedLoadingData')
    ...
    generateColorRamp(colorRamp: string, buckets: number, flip: boolean = false): string[] {
      ...
      try {
        if (d3[`scheme${colorRamp}`] && d3[`scheme${colorRamp}`][buckets]) {
          ...
          // Berechnung der Buckets nur für colormap
          const step = 256 / (buckets - 1)
          const generatedColors = []
          for (let i = 0; i < buckets - 1; i++) {
            generatedColors.push(colors256[Math.round(step * i)])
          }
          generatedColors.push(colors256[255])
          colors256 = generatedColors
        }
      } catch (error) {
        console.error(`Fehler beim Laden der Farbskala ${colorRamp}:`, error)
        return []
      }
    ...
    hexToRgb(hex: Rgb[]): Rgb[] {
      return hex.map(color => {
        if (typeof (color as any) === 'string') {
          let hexStr = (color as unknown as string).replace(/^#/, '')
          ...
          if (hexStr.length !== 6) {
            throw new Error(`Unknown HEX color value: ${hexStr}`)
          }
        ...
        return color // Falls es bereits ein Rgb-Objekt ist
      })
    },
    ...
    rgbToHexSingle(r: number, g: number, b: number) {
    ...
    setLegend(colors: any[], breakpoints: number[]) {
      ...
      this.legendStore.setLegendSection({
        ...
        values: colors.map((rgb, index) => {
          ...
          return { label, value: this.hex2rgb(rgb) }
        }),
      })
    ...
    setupGui() {
      ...
      // breakpoints.add(this.guiConfig, 'Custom breakpoints...', [10, 100, 1])
    },
    ...
    async getVizDetails() {
      if (this.config) {
        ...
        // TODO: REWRITE (currently nor correct)
        //
        // Wo have to check 6 different variations of the definition of the breakpoints and the colorRamp
        //
        // Use cases for the breakpoints:
        //
        // a) breakpoints as an object with values and colors (e.g. { values: [0, 1, 2, 3, 4], colors: ['#ff0000', '#00ff00', '#0000ff', '#ffff00'] })
        // b) breakpoints as an array of numbers (e.g. [0, 1, 2, 3, 4])
        // c) breakpoints are not defined in the yaml
        //
        // Use cases for the colorRamp:
        //
        // 1) colorRamp is defined in the yaml (e.g. colorRamp: 'Viridis')
        // 2) colorRamp is not defined in the yaml
        //
        // This should happen for each of the combinations of the use cases above
        //
        // 1a) breakpoints as an object with values and colors and colorRamp is defined in the yaml:
        //
        // the colorRamp should be ignored and the colors from the breakpoints object should be used
        //
        // 1b) breakpoints as an array of numbers and colorRamp is defined in the yaml:
        //
        // the colorRamp should be used and the colors should be generated based on the colorRamp and the breakpoints
        //
        // 1c) breakpoints are not defined in the yaml and colorRamp is defined in the yaml:
        //
        // use the default number of breakpoints and generate colors based on the colorRamp
        //
        // 2a) breakpoints as an object with values and colors and colorRamp is not defined in the yaml:
        //
        // same as 1a)
        //
        // 2b) breakpoints as an array of numbers and colorRamp is not defined in the yaml:
        //
        // use the default colorRamp (Viridis) and generate colors based on the colorRamp and the breakpoints
        //
        // 2c) breakpoints are not defined in the yaml and colorRamp is not defined in the yaml:
        //
        // use the default number of breakpoints and generate colors based on the default colorRamp (Viridis)

        this.vizDetails.colorRamp = Object.assign(
          ALL_COLOR_RAMPS.filter(ramp => ramp.ramp == 'Viridis')[0],
          { steps: DEFAULT_NUMBER_OF_BUCKETS }
        )
        ...
        // Ignore colorRamp id breakpoints are defined
        if (this.config.breakpoints) {
          if (Array.isArray(this.config.breakpoints)) {
            // console.log('Variation 1')
            // console.log('Breakpoints', this.config.breakpoints)
            this.vizDetails.colorRamp.breakpoints = this.config.breakpoints
            this.vizDetails.colorRamp.steps = this.config.breakpoints.length + 1

            // @ts-ignore
            delete this.vizDetails.breakpoints
            // delete this.vizDetails.colorRamp.customRamp

            // Use case 1b)
          } else if (this.config.breakpoints.values && this.config.breakpoints.colors) {
            // console.log('Variation 2')
            this.vizDetails.colorRamp.breakpoints = this.config.breakpoints.values
            this.vizDetails.colorRamp.customRamp = this.config.breakpoints.colors
            this.vizDetails.colorRamp.steps = this.config.breakpoints.colors.length
            ...
            // TODO Add error message or default values
          }
          // console.log('Viz Details', this.vizDetails)
        } else if (this.config.colorRamp) {
          this.vizDetails.colorRamp = Object.assign(
            ALL_COLOR_RAMPS.filter(ramp => ramp.ramp == this.config?.colorRamp)[0],
            { steps: DEFAULT_NUMBER_OF_BUCKETS }
          ) as Ramp
        } else {
          this.vizDetails.colorRamp = Object.assign(
    ...
    setVizDetails() {
      ...
      if (this.vizDetails.colorRamp) this.guiConfig['color ramp'] = this.vizDetails.colorRamp.ramp
    },
  },
})
