...
import type { ProvidedEntity } from './hdf5-models'
...
async function getEntity(fileId: bigint, path: string): Promise<ProvidedEntity> {
  const h5wasm = await h5wasmReady
...
async function getAttrValue(fileId: bigint, path: string, attrName: string): Promise<unknown> {
  return new Attribute(fileId, path, attrName).json_value
}

async function getDescendantPaths(fileId: bigint, rootPath: string) {
  const h5wasm = await h5wasmReady
  return h5wasm.get_names(fileId, rootPath, true).map(path => `${rootPath}${path}`)
}
